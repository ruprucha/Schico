...
uniq_launch_sites = spacex_df['Launch Site'].unique()

#create dropdown options
dropdown_options = [{'label':'All Sites','value':'ALL'} ] + [{'label':site,'value':site} for site in uniq_launch_sites]
...
def scatter_plot(entered_site,payload_range):
    if entered_site=='ALL':
        ...
        fig = px.scatter(spacex_df[mask],x='Payload Mass (kg)',y='class',color='Booster Version Category',title='Payload vs. Outcome for All Sites',)
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]
        low,high=payload_range
        mask = (filtered_df['Payload Mass (kg)']>low)&(filtered_df['Payload Mass (kg)']<high)
        fig = px.scatter(filtered_df[mask],x='Payload Mass (kg)',y='class',color='Booster Version Category',title=f'Payload vs. Outcome for {entered_site}')
        return fig
# Run the app
